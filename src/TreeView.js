...
class TreeView extends Component {
  constructor(props) {
    ...
    this.jsonRenderer = React.createRef()
  }
  ...
  findPath(self, e) {
    ...
    $(nodes).each(function (i, node) {
      ...
      if (
        ...
        var parentKey =
          $(node).parent('OL').parent('li').find('span.property').eq(0).text() +
          '[' +
          $(node).index() +
          ']'
        keys.push(self.getArrayIndex(parentKey.replace(/"+/g, '')))
      ...
      <div>
        ...
        >
          <ul className="copyMenu">
            <li>
              <a onClick={this.copy.bind(this, event, 'path')}>Copy path</a>
            </li>
            <li>
              <a onClick={this.copy.bind(this, event, 'value')}>Copy Value</a>
            </li>
          </ul>
        </a>
        <pre ref={this.jsonRenderer} id="json-rb"></pre>
      </div>
    )
  }
}
